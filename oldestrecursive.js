//2/26/25 :  before chatgpt refactor
class RecursiveClass {
    ...
    initialInsertDoThisFirst(rowIndex, colIndex, grid, leftOverChar, fromIndex) {
      ...
      //when bottom row on right end has a character that creates an additional row
      ...
      //leftoverchar is the character being inserted (from index)
      combineTopRow = [...leftTopRow, ...leftOverChar,  ...rightTopRow];
      if(verticalCursorPosition/10 == rowIndex){
        
      }
      }else{
        //leftoverchar is now remainder, so it is added to the front
      ...
      //get the firstt row of characters and the remainder to pass into the recursion
