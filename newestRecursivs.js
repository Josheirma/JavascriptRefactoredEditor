...
class RecursiveClass {
  ...
  getLastSpaceOrDash(grid, topRow) {
    ...
    const maxIndex =
      Math.max(lastIndexOfDash, lastIndexOfSpace, -1) === -1
        ? topRow.length
        : Math.max(lastIndexOfDash, lastIndexOfSpace);
  ...
  pushWordsDoThisSecond(grid, newRemainder, rowIndex, fromIndex) {
    ...
    let { rightSide: wordAtEndOfRowOne } = this.getLastSpaceOrDash(
      grid,
      topRow
    );
    ...
    let [firstWordBottomRow, phraseAfterLeftWordBottomRow] = this.splitAtIndex(
      bottomRow,
      lastIndexOfFirstWord
    );
    let lengthOfFirstWordBottomRow = firstWordBottomRow.length;
    let remainingNullSpaces = this.countRemainingNullsAndSpaces(
      grid,
      rowIndex,
      lengthOfFirstWordBottomRow
    );
    ...
    if (
      lengthOfRightWordAtRowOne <= remainingNullSpaces &&
      lengthOfRightWordAtRowOne !== 0
    ) {
      let totalLeftSidePhraseLength =
        lengthOfRightWordAtRowOne + lengthOfFirstWordBottomRow;
      let [leftmostTotalWordThatIsRemoved, restOfBottomRowWithoutLeftWord] =
        this.splitAtIndex(bottomRow, totalLeftSidePhraseLength);
      let combinedLowerRow = [
        ...wordAtEndOfRowOne,
        ...firstWordBottomRow,
        ...restOfBottomRowWithoutLeftWord,
      ];
      if (verticalCursorPosition / 10 === rowIndex) {
        horizontalCursorPosition = totalLeftSidePhraseLength * 5;
      }
      grid[rowIndex] = combinedLowerRow;
      this.replaceTopRowOnlyWithDashes(
        grid,
        rowIndex,
  ...
  divideNextRowsAsNeeded(grid, colIndex, rowIndex, remainder = []) {
    ...
    const [bottomLeftRow, bottomRightRow] = this.splitAtIndex(
      bottomRow,
      colIndex
    );
    const [leftTopRow, rightTopRow] = this.splitAtIndex(topRow, colIndex);
    const buildNextRow = this.combineRowsWithRemainder(
      rightTopRow,
      bottomRow,
      remainder
    );
    ...
    if (newRemainder.length > 0) {
      ...
      return this.divideNextRowsAsNeeded(
        grid,
        colIndex,
        rowIndex + 1,
        newRemainder
  ...
  handleLastRow(rowIndex, columnIndex, grid) {
    ...
    let combine = [];
    if (columnIndex !== 0) {
      let [topRowLeft, topRowRight] = this.splitAtIndex(
        topRow,
        columnIndex - 1
      );
      let [, topRowRightWithoutFirstCharacter] = this.splitAtIndex(
        topRowRight,
        1
      );
      combine = [...topRowLeft, ...topRowRightWithoutFirstCharacter];
    } else {
      let [, topRowRightWithLeftCharacterRemoved] = this.splitAtIndex(
        topRow,
        1
      );
  ...
  removeLeftCharacterFrom2ndRowAndReplaceAboveOnMostRightSide(
    ...
    let [topRowWithoutRightCharacter] = this.splitAtIndex(
      topRow,
      topRow.length - 1
    );
    // Build new top row by appending the leftmost character of the bottom row
    let newTopRow = [
      ...topRowWithoutRightCharacter,
